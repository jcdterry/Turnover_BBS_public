...
setwd('~/Documents/Birds_2024')

RunID =  as.numeric(Sys.getenv("SLURM_ARRAY_TASK_ID"))

## Load Data
Average_Curves<- read.csv('All_Average_Curves_3.csv')
nrow(Average_Curves)

# Average_Curves<- read_csv('../Data/All_Average_Curves.csv')
STATS_select  <- read.csv('STATS_select_SCALED.csv')
BCRsToFit     <- read.csv('BCRsToFit.csv')
